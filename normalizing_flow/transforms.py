...
#         inv_log_det_jac = scale.log().view(x.size(0), -1).sum(dim=1)
#         return inv_y, inv_log_det_jac
#
#     def invert(self, y: torch.tensor):
#         """Computes inverse transformation and log abs determinant of jacobian matrix.
...
#         scale = torch.sigmoid(log_scale+2)
#         inv_y_a, inv_y_b = y_a, y_b / scale - bias
#         inv_y = torch.concat([inv_y_a, inv_y_b], dim=1)
#         inv_log_det_jac = -scale.log().view(y.size(0), -1).sum(dim=1)
#         return inv_y, inv_log_det_jac
...
class AffineCoupling(Transform):
    ...
    def transform(self, x: torch.tensor):
        ...
        inv_y = torch.concat([y_a, y_b], dim=1)
        inv_log_det_jac = scale.view(x.size(0), -1).sum(dim=1)
        return inv_y, inv_log_det_jac
...
class Squeeze(Transform):
    ...
    def transform(self, x: torch.tensor):
        ...
        Returns:
            ...
            log_det_jac: log abs determinant of jacobian matrix of the transformation.
        """
        y, log_det_jac = rearrange(x, 'b c (h h1) (w w1) -> b (c h1 w1) h w', h1=2, w1=2), 0.0
        return y, log_det_jac
    ...
    def invert(self, y: torch.tensor):
        ...
        Returns:
            ...
            inv_log_det_jac: log abs determinant of jacobian matrix of the inverse transformation.
        """
        inv_y, inv_log_det_jac = rearrange(y, 'b (c c1 c2) h w -> b c (h c1) (w c2)', c1=2, c2=2), 0.0
        return inv_y, inv_log_det_jac
